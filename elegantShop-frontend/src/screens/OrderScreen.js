...
const OrderScreen = ({ match, history }) => {
  ...
  useEffect(() => {
    ...
    // add mpesa method
    const addMpesaScript = async () => {
      await axios({
        method: 'get',
        url: '/api/config/mpesaConfig',
        // data: {
          // clientId
        // }
      } 
  ...
  if (!order || successPay || successDeliver || order._id !== orderId) {
    ...
    dispatch({ type: ORDER_DELIVER_RESET })
    dispatch(getOrderDetails(orderId))
  } else if (!order.isPaid) addMpesaScript()
            setSdkReady(true)
}, [dispatch, orderId, successPay, successDeliver, order, history, userInfo])
              ...
              {!order.isPaid && (
                <ListGroup.Item>
                  ...
                  {!sdkReady ? (
                    ...
                    // />
                    <Button onSuccess={()=>successPaymentHandler()}>Complete with MPesa</Button>
                  )}
                </ListGroup.Item>
              )}
